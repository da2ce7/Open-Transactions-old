...
std::shared_ptr<OTServerContract> OTWallet::GetServerContractPartialMatch(const std::string PARTIAL_ID)
{
	...
	FOR_EACH(mapOfServers, m_mapServers)
	{
		std::shared_ptr<OTContract> pServer = (*it).second;
		OT_ASSERT_MSG((NULL != pServer), "NULL server pointer in OTWallet::m_mapServers, OTWallet::GetServerContract");
        ...
        if (str_Name.compare(0,PARTIAL_ID.length(),PARTIAL_ID) == 0)
            return std::dynamic_pointer_cast<OTServerContract>(pServer);
	}
    // --------------------------------------
	return nullptr;
}
...
//
std::shared_ptr<OTPseudonym> OTWallet::GetOrLoadPrivateNym(const OTIdentifier & NYM_ID,
                                            const char * szFuncName/*=NULL*/,
...
//
std::shared_ptr<OTPseudonym> OTWallet::GetOrLoadNym(const OTIdentifier & NYM_ID,
                                     const char * szFuncName/*=NULL*/,
                                     const OTString * pstrReason/*=NULL*/)
{
