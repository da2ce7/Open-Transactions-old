...
bool OTSymmetricKey::GetRawKeyFromPassphrase(
	const OTPassword & thePassphrase, 
		  ...
		  std::unique_ptr<OTPassword> & pDerivedKey  // Optionally pass this, to save me the step.
		  ) const
{    
    OT_ASSERT(m_bIsGenerated);
//  OT_ASSERT(thePassphrase.isPassword());
...
bool OTSymmetricKey::GetRawKeyFromDerivedKey(const OTPassword & theDerivedKey, OTPassword & theRawKeyOutput) const
{
	...
	const bool bDecryptedKey = OTEnvelope::Decrypt(
		// We're using theDerivedKey to decrypt m_dataEncryptedKey.
		theDerivedKey,
		// -------------------------------
		...
		// -------------------------------
		// Created when *this symmetric key was generated. Both are already stored.
		m_dataIV,
		// -------------------------------
		// OUTPUT. (Recovered plaintext of symmetric key.) You can pass OTPassword& OR OTPayload& here (either will work.)
		theRawKeyOutput
		);
                                                   ...
                                                   const bool       bAskTwice  /*=false*/) // returns a text OTPassword, or NULL.
{
    ...
    // -----------------------------------------------
    const int nCallback = OTAsymmetricKey::GetPasswordCallback()(pPassUserInput->getPasswordWritable_char(),
