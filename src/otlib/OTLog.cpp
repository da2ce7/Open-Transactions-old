...
static OTString __Version = "0.86.f";  // todo: new version system ?
...
{
	if (nullptr == p_Config)	{ OTLog::vError("%s: Error: %s is NULL!\n",  __FUNCTION__, "p_Config"	); OT_ASSERT(false); return false; }
	if (! strSection.Exists())	{ OTLog::vError("%s: Error: %s is Empty!\n", __FUNCTION__, "strSection"	); OT_ASSERT(false); return false; }
	if (! strKey.Exists())		{ OTLog::vError("%s: Error: %s is Empty!\n", __FUNCTION__, "szKey"		); OT_ASSERT(false); return false; }
	...
	if (!p_Config -> Set_bool(strSection, strKey_IsRelative, bIsRelative,	bConfigNew_IsRelative, strComment)) return false;
