...
//  useage:
//
//	const std::unique_ptr<OT_API> pOTAPI(OT_CTX::It() -> New());
//  
//  afterwards use pOTAPI -> SomeFunction();
//


//unique_ptr<tthread::mutex> OT_CTX::s_p_ZMQ_Mutex = std::unique_ptr<tthread::mutex>(nullptr);
//unique_ptr<OTSocket> OT_CTX::s_p_Socket = std::unique_ptr<OTSocket>(nullptr);

bool OT_CTX::bOTAPI = false;

OT_CTX::OT_CTX() : m_pCrypto(new OTCrypto_OpenSSL()), m_strPidFilePath("")
{
...
{
	std::unique_ptr<OT_API> p_API = std::unique_ptr<OT_API>(new OT_API("client",tFunc));  // tempory hardcode
...
{
	static std::unique_ptr<OT_CTX> pOT_CTX = nullptr;

	if (nullptr == pOT_CTX) pOT_CTX = std::unique_ptr<OT_CTX>(new OT_CTX());
...
const bool OT_CTX::CleanupOTAPI()
{
	...
	// Gotta love the std::unique_ptr's :)
...
bool OT_API::TransportCallback(const std::shared_ptr<OTServerContract> & pServerContract, const OTEnvelope & theEnvelope)
{
	...
	const std::shared_ptr<OTPseudonym> pNym(m_pClient -> m_pConnection -> GetNym());
	...
	if (ascEnvelope.Exists())
	{
		...
		else
		{
			...
			{                
				...
				// **********************************************************************
				std::unique_ptr<OTMessage> pServerReply(new OTMessage());
...
OT_API::OT_API(const OTString & strThreadContext, const OTServerConnection::TransportFunc & tFunc)
  : m_strThreadContext(strThreadContext),
	...
	m_pSocket(std::unique_ptr<OTSocket>(new OTSocket())),

	m_pWallet(std::unique_ptr<OTWallet>(new OTWallet())),

	m_pClient(std::unique_ptr<OTClient>(new OTClient(tFunc))),
...
OT_API::OT_API(const OTString & strThreadContext)
  : m_strThreadContext(strThreadContext),
	...
	m_pClient(std::unique_ptr<OTClient>(new OTClient())),
...
std::shared_ptr<OTPseudonym> OT_API::GetNymByIDPartialMatch(const std::string PARTIAL_ID, const char * szFuncName/*=NULL*/)
{
	// --------------------------------------------------------------------
...
std::shared_ptr<OTServerContract> OT_API::GetServerContractPartialMatch(const std::string PARTIAL_ID, const char * szFuncName/*=NULL*/)
{
	// --------------------------------------------------------------------
...
const bool OT_API::Wallet_ChangePassphrase()
{
    ...
    for (int iii = 0; iii < nNymCount; ++iii)
    {
        ...
        {
            std::shared_ptr<OTPseudonym> pNym = m_pWallet->GetOrLoadPrivateNym(NYM_ID, __FUNCTION__); // Remember, we ALREADY know there's a private key...
...
const bool OT_API::Wallet_ExportNym(const OTIdentifier & NYM_ID, OTString & strOutput)
{
	...
	// -----------------------------------------------------
    std::shared_ptr<OTPseudonym> pNym = this->GetOrLoadPrivateNym(NYM_ID, szFunc); // This logs and ASSERTs already.
    if (NULL == pNym) return false;
	// -----------------------------------------------------
...
const bool OT_API::Wallet_ImportNym(const OTString & FILE_CONTENTS, OTIdentifier * pNymID/*=NULL*/)
{
	...
	// -----------------------------------------------------
	std::shared_ptr<OTPseudonym> pNym = this->GetOrLoadPrivateNym(theNymID, szFunc); // This logs and ASSERTs already.
    ...
    //
    pNym = std::shared_ptr<OTPseudonym>(new OTPseudonym(theNymID));
    OT_ASSERT(NULL != pNym);
...
const bool OT_API::Wallet_ImportCert(const OTString & DISPLAY_NAME, const OTString & FILE_CONTENTS, OTIdentifier * pNymID/*=NULL*/)
{
    ...
    if (bIfNymLoadKeys && pNym->SetIdentifierByPubkey())
    {
        ...
        // -----------------------------------------------------
        std::shared_ptr<OTPseudonym> pTempNym = this->GetOrLoadPrivateNym(pNym->GetConstID(), szFunc); // This logs and ASSERTs already.
...
///
std::shared_ptr<OTPseudonym> OT_API::GetOrLoadPrivateNym(const OTIdentifier & NYM_ID, const char * szFuncName/*=NULL*/, const OTString * pstrReason/*=NULL*/)
{
	if (nullptr == m_pWallet) return std::shared_ptr<OTPseudonym>();

	return m_pWallet -> GetOrLoadPrivateNym(NYM_ID, szFuncName,pstrReason); 
}
...
///
std::shared_ptr<OTPseudonym> OT_API::GetOrLoadNym(const OTIdentifier & NYM_ID, const char * szFuncName/*=NULL*/, const OTString * pstrReason/*=NULL*/)
{
	if (nullptr == m_pWallet) return std::shared_ptr<OTPseudonym>();

	return m_pWallet -> GetOrLoadNym(NYM_ID, szFuncName,pstrReason); 
}
...
//  OTPasswordData thePWData(strReason);
	...
	// -----------------------------------------------------------------
	std::shared_ptr<OTPseudonym> pNym = this->GetOrLoadPrivateNym(USER_ID, szFunc, &strReason); // These copiously log, and ASSERT.
    if (NULL == pNym) return NULL;
	// -----------------------------------------------------------------
...
//  OTPasswordData thePWData(strReason);
	// -----------------------------------------------------
    std::shared_ptr<OTPseudonym> pOwnerNym = nullptr; // In the case where there is an owner, this will point to it.
	// -----------------------------------------------------
...
//  OTPasswordData thePWData(strReason);
	...
	if (strPurse.Exists() && thePurse.LoadContractFromString(strPurse))
	{
                 ...
                 )
        {
            ...
            // ---------------------------------------------------
            std::shared_ptr<OTPseudonym> pOwnerNym = bCanBePublic ? this->GetOrLoadNym(*pActualOwnerID, szFunc, &strReason) :
...
//  OTPasswordData thePWData(strReason);
    ...
    OTPassword thePassword; // Only used in the case of password-protected purses.
	// -----------------------------------------------------
	std::shared_ptr<OTPseudonym> pNym = this->GetOrLoadPrivateNym(SIGNER_ID, szFunc, &strWalletReason); // These copiously log, and ASSERT.
	if (NULL == pNym) return false;
	// By this point, pNym is a good pointer, and is on the wallet. (No need to cleanup.)
	// -----------------------------------------------------
...
//  OTPasswordData thePWData(strReason);
	// -----------------------------------------------------
	std::shared_ptr<OTPseudonym> pSignerNym = this->GetOrLoadPrivateNym(SIGNER_NYM_ID, szFunc, &strWalletReason); // These copiously log, and ASSERT.
    ...
    std::shared_ptr<OTPseudonym> pOldNym = nullptr;
    ...
    std::shared_ptr<OTPseudonym> pNewNym = nullptr;
...
{	
	...
	const OTString strAssetTypeID(ASSET_ID);
	// -------------------------------------------------------------
	std::shared_ptr<OTServerContract>  pServerContract	= this->GetServer(SERVER_ID, szFunc);
...
//
std::shared_ptr<OTMessage> OT_API::PopMessageBuffer(const long         &   lRequestNumber,
                                     const OTIdentifier &	SERVER_ID,
                                     const OTIdentifier &	USER_ID)
{
...
std::shared_ptr<OTMessage> OT_API::GetSentMessage(const long         & lRequestNumber,
                                   const OTIdentifier &	SERVER_ID,
                                   const OTIdentifier &	USER_ID)
{
	OT_ASSERT_MSG( (m_bInitialized && (nullptr != m_pClient)) , "Not initialized; call OT_API::Init first.");
                                     ...
                                     OTLedger     & THE_NYMBOX)
{
    ...
    FOR_EACH(mapOfTransactions, THE_NYMBOX.GetTransactionMap())
    {
        ...
        {
			std::shared_ptr<OTMessage> pMessage = m_pClient -> GetMessageOutbuffer().GetSentMessage(*pTransaction);
						 ...
						 OTString		& BASKET_INFO)
{
	...
	const std::unique_ptr<OTMessage> pMessage(new OTMessage());
...
{	
			 ...
			 theEnvelope.GetAsciiArmoredData(pMessage -> m_ascPayload))
	{
		...
		//
		std::unique_ptr<OTMessage> pOutMessage(new OTMessage());
...
{	
	...
	// -----------------------------------------------------
	std::shared_ptr<OTPseudonym> pNym = this->GetOrLoadPrivateNym(USER_ID, szFunc); // This ASSERTs and logs already.
	if (NULL == pNym) return (-1);	
	// By this point, pNym is a good pointer, and is on the wallet.
	//  (No need to cleanup.)
	// -----------------------------------------------------
	std::shared_ptr<OTServerContract> pServer = this->GetServer(SERVER_ID, szFunc); // This ASSERTs and logs already.
	if (NULL == pServer) return (-1);
	// By this point, pServer is a good pointer.  (No need to cleanup.)
	// -----------------------------------------------------
	const std::unique_ptr<OTMessage> pMessage(new OTMessage());
