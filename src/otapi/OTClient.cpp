...
//
	...
	std::unique_ptr<OTMessage> pMsg(new OTMessage()); // a copy.
    OT_ASSERT(NULL != pMsg);
            ...
            else    // But if we HAVEN'T already seen the server's reply, then lucky for us he dropped a copy into the Nymbox! Now we can process it!
            {
                    ...
                    OTItem::acknowledgement == pItem->GetStatus())
                {
                    ...
                    else // strOriginalReply.Exists() == true.
                    {
                        ...
                        OT_ASSERT_MSG(pMessage != NULL, "OTClient::AcceptEntireNymbox: std::unique_ptr<OTMessage> pMessage(new OTMessage());");
...
///
void OTClient::ProcessIncomingTransactions(const std::unique_ptr<OTWallet> & pWallet, const std::shared_ptr<OTMessage> & s_pReply)
{
...
void OTClient::ProcessPayDividendResponse(OTTransaction & theTransaction, const std::shared_ptr<OTMessage> & s_pReply)
{
...
void OTClient::ProcessDepositResponse(OTTransaction & theTransaction, const std::shared_ptr<OTMessage> & s_pReply)
{
...
void OTClient::ProcessWithdrawalResponse(const std::unique_ptr<OTWallet> & pWallet, OTTransaction & theTransaction, const std::shared_ptr<OTMessage> & s_pReply)
{
...
///
bool OTClient::ProcessServerReply(const std::unique_ptr<OTWallet> & pWallet, std::unique_ptr<OTMessage> pReply, OTLedger * pNymbox/*=NULL*/) // IF the Nymbox is passed in, then use that one, where appropriate, instead of loading it internally.
{
	...
	if (!s_pReply -> VerifySignature(*pServerNym)) 
	{
		...
		s_pReply = std::unique_ptr<OTMessage>(nullptr);
	...
	std::shared_ptr<OTMessage> pSentMsg = this->GetMessageOutbuffer().GetSentMessage(atol(s_pReply -> m_strRequestNum.Get()),
		strServerID,
	...
	else if (s_pReply -> m_bSuccess && s_pReply -> m_strCommand.Compare("@getNymbox"))
	{
		...
		if (theNymbox.LoadNymboxFromString(strNymbox)) // && theNymbox.VerifyAccount(*pServerNym)) No point doing this, since the client hasn't even had a chance to download the box receipts yet. (VerifyAccount will fail before then...)
		{
			...
			//                {
			//                    std::shared_ptr<OTMessage> pMessage = this->GetSentMessage(*pTransaction);
			//                    
