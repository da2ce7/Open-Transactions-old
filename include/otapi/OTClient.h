...
public:
	...
	bool ProcessServerReply(const std::unique_ptr<OTWallet> & pWallet, std::unique_ptr<OTMessage> pReply, OTLedger * pNymbox=NULL); // IF the Nymbox is passed in, then use that one, where appropriate, instead of loading it internally.

	//these dosn't take owniship of pReply, however whatever calls it must have owniship.
	void ProcessIncomingTransactions(const std::unique_ptr<OTWallet> & pWallet, const std::shared_ptr<OTMessage> & s_pReply);  
	void ProcessWithdrawalResponse(const std::unique_ptr<OTWallet> & pWallet, OTTransaction & theTransaction, const std::shared_ptr<OTMessage> & s_pReply);
	void ProcessDepositResponse(OTTransaction & theTransaction, const std::shared_ptr<OTMessage> & s_pReply);
	void ProcessPayDividendResponse(OTTransaction & theTransaction, const std::shared_ptr<OTMessage> & s_pReply);
