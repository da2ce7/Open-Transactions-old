...
EXPORT	std::shared_ptr<OTPseudonym>		GetNymByIDPartialMatch(const std::string PARTIAL_ID);  // wallet name for nym also accepted.
...
EXPORT	std::shared_ptr<OTServerContract>	GetServerContractPartialMatch(const std::string PARTIAL_ID);  // wallet name for server also accepted.
